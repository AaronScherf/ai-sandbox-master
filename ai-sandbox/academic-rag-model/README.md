...
- **Problem generation sub-agent only** (`problem_gen/`, reached via `rag/`'s automatic intent detection on the question text): a local Ollama install (`ollama serve`) with `qwen2-math:7b` pulled (`ollama pull qwen2-math:7b`) — optional, degrades to falling back to normal Q&A if unavailable, no `GEMINI_API_KEY` or other paid API involved for generation itself (retrieval still uses the existing Gemini client for embeddings, same as every other retrieval call in this project). On CPU-only Ollama, a single request can take several minutes to up to ~30 minutes in the worst case (a full problem-plus-solution generation is a much longer response than `viz/`'s short code snippets, and up to 3 retries each make two model calls) — see `docs/2026-09-05-problem-generation-status.md` for real measured timings.
